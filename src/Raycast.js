...
export class ray{
	...
	static collisionPoint(rayA, rayB, checkBounds = true){
		//returns the collision point of two rays
		//null if no collision
		var intersection = ray.intersect(rayA, rayB);
		if(!intersection) return null;
		if(checkBounds && !rayA.pointInBounds(intersection)) return null;
		if(checkBounds && !rayB.pointInBounds(intersection)) return null;
		return intersection;
	}
	...
	static bodyCollisions(rayA, body){
        ...
        if (!r.length) {
            for(var i = edges.length - 1; i >= 0; i--){
                //gets the collision point
                var colpoint = ray.collisionPoint(rayA, edges[i], false);
...
export class vec2{
    ...
    toPhysVector(){
        ...
        //Phaser.Physics.Matter.Matter.js physics engine
